...
The answer is wholeheartedly, no. Repeated testing of the data will greatly inflate the type 1 error rate. Instead of only a 1/20 probability to wrongly reject the null hypothesis when it is true, the actual error rate of this sequential testing procedure will be much higher. In theory, repeated sampling is guaranteed to reach a significant result in sampling continues indefinitely. As stated in Edwards, Lindman, and Savage (1963): "And indeed if an experimenter uses this procedure, then with probability 1 he will eventually reject any sharp null hypothesis, even though it be true." 
...
## Simulation using R
Let's simulate this using R and see for ourselves. To recap, we have 20 independent scientists conducting their own experiment. After every data point comes in, they conduct a statistical testing using the data they have collected so far. As we can see by the wiggling lines, given that the null hypothesis is true, the p-value fluctuate wildly with each test. When a scientist obtains a p-value < 0.05, the line past that observation is colored red so that we can identify when significance has been declared by each scientist.

<aside>
The animations were created using the [gganimate](https://gganimate.com/articles/gganimate.html) package.
</aside>

<video src="pval_anim.mp4" height=500 controls></video>
...
<video src="pval_2000.mp4" height=500 controls></video>

As seen before, the <span style="color:#ed6663">cumulative type I error rate</span> inflates indefinitely, reach almost 50% by 1,000 observations, while the <span style="color:#43658b">type I error rate at any given observation</span> remains controlled at the nominal value of 0.05.
